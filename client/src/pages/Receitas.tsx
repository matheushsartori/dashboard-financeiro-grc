...
export default function Receitas() {
  ...
  // Dados para gráfico de receitas por filial
  const receitasPorFilialChart = useMemo(() => {
    if (!receitasPorFilial || receitasPorFilial.length === 0) return [];

    return receitasPorFilial.map(item => ({
      name: item.nomeFilial,
      valor: item.totalRecebido / 100, // Converter de centavos para reais
  ...
  return (
    <div className="container max-w-7xl py-8">
      ...
      {receitasPorFilialChart.length > 0 && (
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>Receitas por Filial</CardTitle>
            <CardDescription>
              Comparação de receitas acumuladas entre as filiais {selectedMonth ? `do mês ${selectedMonth}` : "geral"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={350}>
              <BarChart data={receitasPorFilialChart} margin={{ top: 30, right: 30, left: 20, bottom: 60 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(229, 231, 235, 0.3)" />
                <XAxis
                  dataKey="name"
                  stroke="#6b7280"
