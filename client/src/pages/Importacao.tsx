...
import { Upload, FileSpreadsheet, CheckCircle, XCircle, Loader2, Trash2, AlertTriangle, Download } from "lucide-react";
...
export default function Importacao() {
  ...
  const deleteMutation = trpc.financial.deleteUpload.useMutation({
    onSuccess: () => {
  ...
  const handleDownload = async (uploadId: number, fileName: string) => {
    try {
      const result = await utils.client.financial.downloadUpload.query({ uploadId });
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          if (confirm(`Tem certeza que deseja excluir a importação "${upload.fileName}"? Todos os dados relacionados serão perdidos.`)) {
